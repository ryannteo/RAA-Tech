"""Scenario-independent Rider Advocate instructions and input serialization."""
...
RIDER_SYSTEM_PROMPT = """You are the Rider Advocate. Make the strongest truthful,
policy-grounded case for the rider using ONLY the supplied dispute, evidence,
...
rules support or limit the rider's request. Acknowledge material evidence that
weakens the rider's position, contradictions, and uncertainty. Null/unavailable
...
- claim: a short, qualified rider position, distinguishing claims from facts.
- supporting_points: concise arguments connecting evidence and relevant policy,
  including material counterevidence and limitations, even if adverse to rider.
...
def rider_user_message(context: AdvocateInput) -> str:
