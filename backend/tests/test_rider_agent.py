"""Deterministic Rider contracts and workflow failures, not prose-quality evals."""
import json
...
from app.agents.rider_prompt import RIDER_SYSTEM_PROMPT
...
def case_data(rule_id="RD-01"):
    return {
        "side": "rider", "source": "llm",
        "claim": "Rider requests an evidence-based assessment.",
        "supporting_points": [
...
async def test_untrusted_statements_and_chat_stay_in_data_message():
    ...
    await RiderAdvocateAgent(fake).run(context)
...
async def test_malformed_output_is_rejected_without_fallback(output):
    ...
    with pytest.raises(ValidationError):
        await RiderAdvocateAgent(fake).run(context_for())
    assert len(fake.calls) == 1
...
async def test_explicit_mock_mode_is_labelled_and_never_calls_provider(monkeypatch):
    ...
    result = await RiderAdvocateAgent(client).run(context_for())
...
async def test_input_revalidated_before_provider_call():
    ...
    fake = FakeLLM(json.dumps(case_data()))
    with pytest.raises(ValidationError):
        await RiderAdvocateAgent(fake).run(invalid)
...
def forbid_downstream(monkeypatch):
    async def forbidden(context):
        pytest.fail("Neither Driver nor Judge may run after Rider failure")

    monkeypatch.setattr(graph.driver_advocate_agent, "run", forbidden)
...
async def test_real_rider_failure_stops_workflow(monkeypatch, failure):
    monkeypatch.setattr(graph, "rider_advocate_agent", RiderAdvocateAgent(failed_client(failure)))
...
def test_api_failure_is_controlled_and_not_stored(monkeypatch, failure):
    ...
    with TestClient(app) as client:
        ...
        assert detail["stage"] == "rider_advocate"
        assert "private" not in response.text
        assert client.get("/disputes").json() == []


def test_api_success_keeps_existing_public_contract_and_remaining_stubs(monkeypatch):
    monkeypatch.setattr(graph, "rider_advocate_agent", RiderAdvocateAgent(FakeLLM(json.dumps(case_data()))))
    monkeypatch.setattr(routes, "store", InMemoryDisputeStore())
    with TestClient(app) as client:
        response = client.post("/disputes", json={
            "scenario_id": "route_deviation_001", "category": "route_deviation",
        ...
        body = response.json()
        assert body["rider_case"] == case_data()
        assert body["driver_case"]["source"] == body["ruling"]["source"] == "stub"
